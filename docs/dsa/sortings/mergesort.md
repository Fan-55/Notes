...
**Output:** array $a$ with the same elements that are sorted in ascending order.

**base case**: return if the length of the input array is $1$ or $0$ (already sorted)

$c :=$ recursively sort first half of $a$

$d :=$ recursively sort 2nd half of $a$

**Merge** the sorted $c$ and $d$ and then return the merged result.

## Implementation

### Merge step implementation (see merge method)
...
**Implementations:** [Code snippet](https://github.com/Fan-55/Algorithms/blob/main/src/Merge.java)
